# 5. Перед вами программа, которая умеет высчитывать расстояние от одной точки на карте до другой.
# В коде описан класс Point (точка на карте), у него есть свойства — широта и долгота, и метод distance(self, other) —
# это расстояние между двумя точками в километрах (параметр other должен получить другой объект Point).
...
# Подсказка
# В конструкторе класса City присвойте значения параметров свойствам self.name и self.population.
# Затем вызовите конструктор родительского класса с аргументами «широта» и «долгота»: super().__init__(latitude, longitude)
# Конструктор класса Mountain должен принять на вход широту, долготу, название горы и её высоту: def __init__(self, latitude, longitude, name, height):
